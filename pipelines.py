            ...
            if res[1] != '3' and res[0] not in [0, 6, 7]:
                if status:
                    if str(status) != '3':
                        ...
                        upSql = f"UPDATE dc_travel_business_list SET status='{status}', update_time={int(time())} " \
                            f"WHERE tid={tid};"
                ...
                if submit_status == '111':
                    if res[0] == 1:
                        upSql = f"UPDATE dc_travel_business_list SET repatriation_pdf='', repatriations_pdf=''," \
                            f" submit_status='111' WHERE tid={tid};"
                    else:
                        upSql = f"UPDATE dc_travel_business_list SET status='7', repatriation_pdf='', " \
                            f"repatriations_pdf='', submit_status='111' WHERE tid={tid};"
                    self.cur.execute(upSql)
                elif submit_status in ['221', '222'] and not res[2] and pdf:
                    upSql = f"UPDATE dc_travel_business_list SET repatriation_pdf='{pdf}' WHERE tid={tid};"
                    self.cur.execute(upSql)
            elif res[0] in [6, 7] and submit_status == '211':
                upSql = f"UPDATE dc_travel_business_list SET status='6', submit_status='3', repatriation_pdf='{pdf}' " \
                    f"WHERE tid={tid};"
                self.cur.execute(upSql)
